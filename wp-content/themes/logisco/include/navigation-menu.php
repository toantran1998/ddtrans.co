<?php
	...
	*/
	...
	// custom menu
	if( !function_exists('logisco_get_custom_menu') ){
		function logisco_get_custom_menu( $settings = array() ){
			if( !empty($settings['type']) ){
				if( $settings['type'] == 'overlay' ){
					logisco_get_overlay_menu($settings);
				}else if( $settings['type'] == 'left' || $settings['type'] == 'right' ){
					$settings['slide'] = $settings['type'];
					logisco_get_mmenu($settings);
				}
			}
		}
	}
	...
	// menu icon
	if( !function_exists('logisco_get_mobile_menu_icon') ){
		function logisco_get_mobile_menu_icon( $settings = array() ){

			$settings = wp_parse_args($settings, array(
				'href' => '#',
				'button-type' => logisco_get_option('general', 'right-menu-style', 'hamburger-with-border'),
 				'button-class' => '',
				'icon-class' => 'icon_menu'
			));

			$button_class  = $settings['button-class'];
			$button_class .= ' logisco-mobile-button-' . $settings['button-type'];
	...
	// overlay menu
	if( !function_exists('logisco_get_overlay_menu') ){
		function logisco_get_overlay_menu( $settings = array() ){
			...
			echo '<div class="logisco-overlay-menu ' . esc_attr($settings['container-class']) . '" id="' . esc_attr($settings['id']) . '" >';
			
			$settings['button-class'] = 'logisco-overlay-menu-icon ' . $settings['button-class'];
			logisco_get_mobile_menu_icon($settings);

			echo '<div class="logisco-overlay-menu-content logisco-navigation-font" >';
			echo '<div class="logisco-overlay-menu-close" ></div>';

			echo '<div class="logisco-overlay-menu-row" >';
			echo '<div class="logisco-overlay-menu-cell" >';
			wp_nav_menu(array(
	...
	// mmenu
	if( !function_exists('logisco_get_mmenu') ){
		function logisco_get_mmenu( $settings = array() ){
			...
			$settings['button-class'] = 'logisco-mm-menu-button ' . $settings['button-class'];
			$settings['href'] = '#' .  $settings['id'];
			logisco_get_mobile_menu_icon($settings);

			echo '<div class="logisco-mm-menu-wrap logisco-navigation-font" id="' . esc_attr($settings['id']) . '" data-slide="' . esc_attr($settings['slide']) . '" >';
			wp_nav_menu(array(
				'theme_location'=>$settings['theme-location'], 
				'container'=> '', 
				'menu_class'=> 'm-menu'
	...
	if( class_exists('gdlr_core_edit_nav_menu') ){
		new gdlr_core_edit_nav_menu(array(
			'icon-class' => array(
				'title' => esc_html__('Menu Icon Class', 'logisco'),
				'type' => 'text',
			),
			'enable-mega-menu' => array(
				'title' => esc_html__('Enable Mega Menu', 'logisco'),
				'type' => 'checkbox',
				'depth' => '0'
			),
			'mega-menu-background' => array(
				'title' => esc_html__('Mega Menu Background', 'logisco'),
				'type' => 'upload',
				'depth' => '0'
			),
			'mega-menu-background-position' => array(
				'title' => esc_html__('Mega Menu Background Position', 'logisco'),
				'type' => 'combobox',
				'options' => array(
					'center' => esc_html__('Center', 'logisco'),
					'top-left' => esc_html__('Top Left', 'logisco'),
					'top-center' => esc_html__('Top Center', 'logisco'),
					'top-right' => esc_html__('Top Right', 'logisco'),
					'center-left' => esc_html__('Center Left', 'logisco'),
					'center-right' => esc_html__('Center Right', 'logisco'),
					'bottom-left' => esc_html__('Bottom Left', 'logisco'),
					'bottom-center' => esc_html__('Bottom Center', 'logisco'),
					'bottom-right' => esc_html__('Bottom Right', 'logisco'),
				),
				'depth' => '0'
			),
			'mega-menu-background-repeat' => array(
				'title' => esc_html__('Mega Menu Background Repeat', 'logisco'),
				'type' => 'combobox',
				'options' => array(
					'cover' => esc_html__('Cover ( full width and height )', 'logisco'),
					'no-repeat' => esc_html__('No Repeat', 'logisco'),
					'repeat' => esc_html__('Repeat X & Y', 'logisco'),
					'repeat-x' => esc_html__('Repeat X', 'logisco'),
					'repeat-y' => esc_html__('Repeat Y', 'logisco'),
				),
				'default' => 'cover',
				'condition' => array( 'background-type' => 'image' )
			),
			'mega-menu-width' => array(
				'title' => esc_html__('Mega Menu Width ( Fill value with % or px )', 'logisco'),
				'type' => 'text',
				'default' => '100%',
				'depth' => '0'
			),
			'hide-menu-title' => array(
				'title' => esc_html__('Hide Menu Title', 'logisco'),
				'type' => 'checkbox',
				'depth' => '1'
			),
			'mega-menu-section-size' => array(
				'title' => esc_html__('Section Size ( Only for mega menu )', 'logisco'),
				'type' => 'combobox',
				'options' => array( 
			...
			'mega-menu-section-content' => array(
				'title' => esc_html__('Section Content ( Only for mega menu )', 'logisco'),
				'type' => 'textarea',
				'depth' => '1'
			),
		));
	...
	if( !class_exists('logisco_menu_walker') ){
		
		// from wp-includes/nav-menu-template.php file
		class logisco_menu_walker extends Walker_Nav_Menu{
			...
			function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
				...
				if( $depth == 0 ){
					...
					if( ceil($this->top_level_items / 2) + 1 == $this->top_level_count ){
						$center_nav_item = apply_filters('logisco_center_menu_item', '');
						if( !empty($center_nav_item) ){
							$output .= '<li class="logisco-center-nav-menu-item" >' . $center_nav_item . '</li>';
						}
					}
				}
				...
				if( $depth == 0 ){
					if( $item->gdlr_core_nav_menu_custom['enable-mega-menu'] == 'disable' ){
						$class_names .= ' logisco-normal-menu';
					}else{
						$class_names .= ' logisco-mega-menu';
					}
				}else if( $depth == 1 ){
				...
				if( $depth == 1 && !empty($item->gdlr_core_nav_menu_custom['mega-menu-section-content']) ){
					$item_output .= '<div class="logisco-mega-menu-section-content">';
