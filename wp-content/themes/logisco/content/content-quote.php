...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<div class="logisco-single-article logisco-blog-quote-format" >
	<?php 
		// post title
		...
		if( !empty($thumbnail_id) ){
			...
			echo '<div class="logisco-blog-quote-background" ' . gdlr_core_esc_style(array(
				'background-image' => $quote_background
			)) . ' ></div>';
		}

		echo '<div class="logisco-blog-quote gdlr-core-quote-font" >&#8220;</div>';
		echo '<div class="logisco-blog-content-wrap" >';
		if( !empty($blockquote) ){
			echo '<div class="logisco-blog-quote-content gdlr-core-info-font">' . gdlr_core_content_filter($blockquote, true) . '</div>';
		}
		if( !empty($author) ){
			echo '<div class="logisco-blog-quote-author gdlr-core-info-font" >' . gdlr_core_escape_content(gdlr_core_text_filter($author)) . '</div>';
		}
		echo '</div>';
		echo '</div>';
	?>
