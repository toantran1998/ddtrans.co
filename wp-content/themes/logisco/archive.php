 ...
 */
	...
	if( is_tax('portfolio_category') || is_tax('portfolio_tag') && class_exists('gdlr_core_pb_element_portfolio') ){
		...
		$sidebar_type = logisco_get_option('general', 'archive-portfolio-sidebar', 'none');
		$sidebar_left = logisco_get_option('general', 'archive-portfolio-sidebar-left');
		$sidebar_right = logisco_get_option('general', 'archive-portfolio-sidebar-right');		
	}else{
		$post_type = 'post';
