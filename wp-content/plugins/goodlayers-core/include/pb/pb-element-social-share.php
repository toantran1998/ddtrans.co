			...
			foreach( $social_list as $social ){
				switch( $social ){
					case 'facebook': 
						...
						$response = wp_remote_get('https://graph.facebook.com/?id=' . trim($url) . '&fields=og_object{engagement}&access_token=' . trim($settings['facebook-access-token']));
						$response_body = wp_remote_retrieve_body($response);

						if( current_user_can('administrator') && isset($_GET['debug_social_count']) ){
							echo 'https://graph.facebook.com/?id=' . trim($url) . '&fields=og_object{engagement}&access_token=' . trim($settings['facebook-access-token']);
						}
