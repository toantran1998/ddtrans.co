...
if (!class_exists('BF_Admin_Page_Class')):
    ...
    class BF_Admin_Page_Class {
        ...
        public function AddMenuTopPage() {
            $default = array('capability' => 'manage_optionss', 'menu_title' => 'Cool Timeline', 'id' => 'id', 'icon_url' => COOL_TIMELINE_PLUGIN_URL . 'assets/images/timeline-icon-small.png', 'position' => 6);
        ...
        public function AddMenuSubPage() {
            $default = array('capability' => 'manage_optionss');
            $this->args = array_merge($default, $this->args);
        ...
        public function plupload_head_js() {
            if ($this->has_field('plupload')) {
                $plupload_init = array('runtimes' => 'html5,silverlight,flash,html4', 'browse_button' => 'plupload-browse-button', // will be adjusted per uploader
                'container' => 'plupload-upload-ui', // will be adjusted per uploader
                'drop_element' => 'drag-drop-area', // will be adjusted per uploader
                'file_data_name' => 'async-upload', // will be adjusted per uploader
                'multiple_queues' => true, 'max_file_size' => wp_max_upload_size() . 'b', 'url' => admin_url('admin-ajax.php'), 'flash_swf_url' => includes_url('js/plupload/plupload.flash.swf'), 'silverlight_xap_url' => includes_url('js/plupload/plupload.silverlight.xap'), 'filters' => array(array('title' => __('Allowed Files', 'apc'), 'extensions' => '*')), 'multipart' => true, 'urlstream_upload' => true, 'multi_selection' => false, // will be added per uploader
...
?>
      <script>
        ...
        var last_tab = getCookie("apc_<?php
            echo $this->option_group;
?>last");
        if (last_tab) {
...
?>
        ...
        public function edit_insert_to_post_text($safe_text, $text) {
            if (is_admin() && 'Insert into Post' == $safe_text) {
                if (isset($_REQUEST['apc']) && 'insert_file' == $_REQUEST['apc']) return str_replace(__('Insert into Post'), __('Use this File', 'apc'), $safe_text);
                else return str_replace(__('Insert into Post'), __('Use this Image', 'apc'), $safe_text);
            }
            return $safe_text;
        }
      ...
      <br style="clear:both"><br>
            ...
            foreach ($this->_fields as $field) {
                ...
                $data = '';
                if (!$this->saved_flag && $saved === false) $data = isset($field['std']) ? $field['std'] : '';
...
<li>Video, Images & Slider</li>
    ...
    <img src="https://res.cloudinary.com/cooltimeline/image/upload/v1468242487/6-buy-cool-timeline_vabou4.png"></a></div>
        ...
        public function check_field_upload() {
            ...
            if (!$this->has_field_any(array('image', 'file'))) return;
        ...
        public function insert_images() {
            ...
            if (!isset($_POST['at-insert']) || empty($_POST['attachments'])) return;
            // Security Check
        ...
        public function wp_ajax_delete_image() {
            ...
            if (strpos($field_id, '[') === false) {
                ...
                if (!$remove_meta_only) $ok = wp_delete_attachment($attachment_id);
                else $ok = true;
            } else {
        ...
        public function reorder_images() {
            ...
            if (!wp_verify_nonce($nonce, 'at_ajax_reorder')) die('1');
        ...
        public function show() {
            ...
            foreach ($this->_fields as $field) {
                ...
                if ('image' != $field['type'] && $field['type'] != 'repeater') $meta = is_array($meta) ? array_map('esc_attr', $meta) : esc_attr($meta);
                echo '<tr>' . "\n";
        ...
        public function show_field_repeater($field, $meta) {
            ...
            $class = '';
            if ($field['sortable']) $class = " repeater-sortable";
            ...
            if (count($meta) > 0 && is_array($meta)) {
                foreach ($meta as $me) {
                    ...
                    foreach ($field['fields'] as $f) {
                        ...
                        if ('image' != $f['type'] && $f['type'] != 'repeater') $m = is_array($m) ? array_map('esc_attr', $m) : esc_attr($m);
                        if (in_array($f['type'], array('text', 'textarea'))) $m = stripslashes($m);
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
        ...
        public function show_field_begin($field, $meta) {
            ...
            if ($field['name'] != '' || $field['name'] != FALSE) echo "<div class='at-label'><label for='{$field['id']}'>{$field['name']}</label></div>\n";
        }
        /**
         * End Field.
         *
        ...
        public function show_field_end($field, $meta = NULL, $group = false) {
            if (isset($field['desc']) && $field['desc'] != '') echo "<div class='desc-field'>{$field['desc']}</div>\n";
        ...
        public function show_field_media_manager($field, $meta) {
            ...
            if (is_array($meta)) {
                if (isset($meta[0]) && is_array($meta[0])) $meta = $meta[0];
            }
        ...
        public function show_field_size($field, $meta) {
            ...
            if (empty($meta['width']) && empty($meta['height'])) {
                $meta = array('width' => $field['std']['width'], 'height' => $field['std']['height']);
            } else {
                if (!is_array($meta)) {
                    $meta = array('width' => '', 'height' => '');
                }
            }
        ...
        public function show_field_typo($field, $meta) {
            $this->show_field_begin($field, $meta);
            if (!is_array($meta)) {
                $meta = array('size' => '', 'face' => '', 'style' => '', 'color' => '#', 'weight' => '');
            }
            ...
            if (isset($meta['size'])) $op = str_replace('value="' . $meta['size'] . '"', 'value="' . $meta['size'] . '" selected="selected"', $op);
            $html.= $op . '</select>' . "\n";
            // Font Face
            ...
            if (isset($meta['face'])) $op = str_replace('value="' . $meta['face'] . '"', 'value="' . $meta['face'] . '" selected="selected"', $op);
            $html.= $op . '</select>' . "\n";
            // Font Weight
            ...
            if (isset($meta['weight'])) $op = str_replace('value="' . $meta['weight'] . '"', 'value="' . $meta['weight'] . '" selected="selected"', $op);
            $html.= $op . '</select>' . "\n";
            /* Font Style */
        ...
        public function show_field_posts($field, $meta) {
            ...
            if ('checkbox_list' == $options['type']) {
                foreach ($posts as $p) {
                    if (isset($field['class']) && $field['class'] == 'no-toggle') echo "<label class='at-posts-checkbox-label'><input type='checkbox' class='at-posts-checkbox" . (isset($field['class']) ? " {$field['class']}" : "") . "' name='{$field['id']}[]' value='$p->ID'" . checked(in_array($p->ID, $meta), true, false) . " /> {$p->post_title}</label>";
                    else echo "{$p->post_title}<input type='checkbox' class='at-posts-checkbox" . (isset($field['class']) ? " {$field['class']}" : "") . "' name='{$field['id']}[]' value='$p->ID'" . checked(in_array($p->ID, $meta), true, false) . " />";
                }
            }
            // select
            else {
        ...
        public function show_field_taxonomy($field, $meta) {
            ...
            if ('checkbox_list' == $options['type']) {
                foreach ($terms as $term) {
                    if (isset($field['class']) && $field['class'] == 'no-toggle') echo "<label class='at-tax-checkbox-label'><input type='checkbox' class='at-tax-checkbox" . (isset($field['class']) ? " {$field['class']}" : "") . "' name='{$field['id']}[]' value='$term->slug'" . checked(in_array($term->slug, $meta), true, false) . " /> {$term->name}</label>";
                    else echo "{$term->name} <input type='checkbox' class='at-tax-checkbox" . (isset($field['class']) ? " {$field['class']}" : "") . "' name='{$field['id']}[]' value='$term->slug'" . checked(in_array($term->slug, $meta), true, false) . " />";
                }
            }
            // select
            else {
        ...
        public function show_field_WProle($field, $meta) {
            ...
            if ($names) {
                ...
                if ('checkbox_list' == $options['type']) {
                    foreach ($names as $n) {
                        if (isset($field['class']) && $field['class'] == 'no-toggle') echo "<label class='at-posts-checkbox-label'><input type='checkbox'  class='at-role-checkbox" . (isset($field['class']) ? " {$field['class']}" : "") . "' name='{$field['id']}[]' value='$n'" . checked(in_array($n, $meta), true, false) . " /> " . $n . "</label>";
                        else echo "{$n} <input type='checkbox'  class='at-role-checkbox" . (isset($field['class']) ? " {$field['class']}" : "") . "' name='{$field['id']}[]' value='$n'" . checked(in_array($n, $meta), true, false) . " />";
                    }
                }
                // select
                else {
        ...
        public function save($repeater = false) {
            ...
            //check nonce
            if (!check_admin_referer(basename(__FILE__), 'BF_Admin_Page_Class_nonce')) return;
            foreach ($this->_fields as $field) {
                if (!in_array($field['type'], $skip)) {
                    $name = $field['id'];
                    $type = $field['type'];
                    ...
                    if (isset($field['validate'])) {
                        if (!$this->validate_field($field, $new)) $new = $old;
                    }
        ...
        public function save_field($field, $old, $new) {
            ...
            if ($new === '' || $new === array() && (!in_array($field['type'], array('text', 'textarea')))) return;
        ...
        public function save_field_image($field, $old, $new) {
            $name = $field['id'];
            unset($this->_saved[$name]);
            if ($new === '' || $new === array() || $new['id'] == '' || $new['src'] == '') return;
            $this->_saved[$name] = $new;
        }
        /*
         * Save Wysiwyg Field.
        ...
        */
        public function save_field_checkbox($field, $old, $new) {
            if ($new === '') $this->save_field($field, $old, false);
            else $this->save_field($field, $old, true);
        }
        /**
        ...
        public function save_field_repeater($field, $old, $new) {
            if (is_array($new) && count($new) > 0) {
                foreach ($new as $n) {
                    ...
                    if (!$this->is_array_empty($n)) $temp[] = $n;
                }
                if (isset($temp) && count($temp) > 0 && !$this->is_array_empty($temp)) {
                    $this->_saved[$field['id']] = $temp;
                } else {
                    if (isset($this->_saved[$field['id']])) unset($this->_saved[$field['id']]);
                }
            } else {
        ...
        public function has_field_any($types) {
            foreach ((array)$types as $t) {
                if ($this->has_field($t)) return true;
            }
            return false;
        }
        /**
        ...
        public function addTypo($id, $args, $repeater = false) {
            $new_field = array('type' => 'typo', 'id' => $id, 'std' => array('size' => '12px', 'color' => '#000000', 'face' => 'arial', 'style' => 'normal', 'weight' => 'normal'), 'desc' => '', 'style' => '', 'name' => 'Typography field');
            $new_field = array_merge($new_field, $args);
            $this->_fields[] = $new_field;
        }
        public function addSizes($id, $args, $repeater = false) {
            $new_field = array('type' => 'size', 'id' => $id, 'std' => array('height' => '100', 'width' => '100'), 'desc' => '', 'style' => '', 'name' => 'Sizes field');
            $new_field = array_merge($new_field, $args);
            $this->_fields[] = $new_field;
        }
        /**
        ...
        public function is_array_empty($array) {
            ...
            foreach ($array as $a) {
                if (is_array($a)) {
                    foreach ($a as $sub_a) {
                        if (!empty($sub_a) && $sub_a != '') return false;
                    }
                } else {
                    if (!empty($a) && $a != '') return false;
                }
            }
            return true;
        }
                ...
                if ($this->google_fonts) {
                    ...
                    if (!is_wp_error($gs)) {
                        ...
                        foreach ($fontArray as $f) {
                            ...
                            $fonts[$key] = array('name' => $f->family, 'import' => str_replace(" ", "+", $f->family), 'css' => 'font-family: ' . $f->family . ';' //@import url(http://fonts.googleapis.com/css?family=
                            );
                        }
                    }
                }
        ...
        public function get_font_style() {
            $default = array('normal' => 'Normal', 'italic' => 'Italic', 'oblique ' => 'Oblique');
        ...
        public function get_font_weight() {
            $default = array('normal' => 'Normal', 'bold' => 'Bold', 'bolder' => 'Bolder', 'lighter' => 'Lighter', '100' => '100', '200' => '200', '300' => '300', '400' => '400', '500' => '500', '600' => '600', '700' => '700', '800' => '800', '900' => '900', 'inherit' => 'Inherit');
    ...
    ';
        ...
        public function export() {
            ...
            if ($options !== false) $re['code'] = "<!*!* START export Code !*!*>\n" . base64_encode(serialize($options)) . "\n<!*!* END export Code !*!*>";
            else $re['err'] = __('error in ajax request! (2)', 'apc');
            //update_nonce
            $re['nonce'] = wp_create_nonce("apc_export");
            echo json_encode($re);
            die();
        ...
        public function download_file($content = null, $file_name = null) {
            if (!wp_verify_nonce($_REQUEST['nonce'], 'theme_export_options')) wp_die('Security check');
            ...
            if (!current_user_can('manage_optionss')) wp_die('<p>' . __('You do not have sufficient permissions to edit templates for this site.', 'apc') . '</p>');
        ...
        public function create_export_download_link($echo = false) {
            ...
            $args = array('theme_export_options' => 'safe_download', 'nonce' => wp_create_nonce('theme_export_options'), 'option_group' => $this->option_group);
            $export_url = add_query_arg($args, $site_url);
            if ($echo === true) echo '<a href="' . $export_url . '" target="_blank">' . __('Download Export', 'apc') . '</a>';
            elseif ($echo == 'url') return $export_url;
        ...
        public function validate_field($field, $meta) {
            if (!isset($field['validate']) || !is_array($field['validate'])) return true;
            $ret = true;
        ...
        public function getFieldErrors($field_id) {
            if ($this->errors_flag) {
                if (isset($this->errors[$field_id])) return $this->errors[$field_id];
            }
        ...
        public function has_error($field_id) {
            ...
            if (!$this->saved_flag || !$this->errors_flag) return false;
            //check if this field has validation errors
            if (isset($this->errors[$field_id])) return true;
            return false;
        }
        /**
         * valid email
